...
export default async function AdminDeliveriesPage({
  ...
  searchParams: Promise<{ deliveries_page?: string; page_size?: string }>;
}) {
  const { locale } = await params;
  const { deliveries_page, page_size } = await searchParams;
  ...
  const where = { status: "CONFIRMED" as const, startDate: { gte: now, lte: sevenDays } };
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <h2 className="mb-4 text-xl font-semibold">{t("admin.dashboard.deliveries.title")}</h2>
      <div className="mb-3 flex flex-col gap-2 text-sm md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {ADMIN_PAGE_SIZE_OPTIONS.map((size) => (
            <Link key={size} className={`inline-flex h-8 items-center rounded-md border px-2 text-xs ${pageSize === size ? "bg-accent font-medium" : "hover:bg-accent"}`} href={buildHref({ page_size: size, deliveries_page: 1 })}>
              {size}
            </Link>
          ))}
        </div>
      ...
      <div className="mt-4 flex items-center justify-end gap-2">
        {safePage > 1 ? (
          <Link className="inline-flex h-8 items-center rounded-md border px-3 text-xs hover:bg-accent" href={buildHref({ deliveries_page: safePage - 1 })}>
            {prevLabel}
          </Link>
        ) : (
