...
export default async function AdminReturnsPage({
  ...
  searchParams: Promise<{ returns_page?: string; page_size?: string; start?: string; end?: string }>;
}) {
  const { locale } = await params;
  const { returns_page, page_size, start, end } = await searchParams;
  ...
  return (
    <div className={ADMIN_PAGE_SHELL}>
      <div className={ADMIN_PAGE_STACK}>
        <p className={ADMIN_PAGE_KICKER}>{t("admin.dashboard.returns.title")}</p>
        <div className={ADMIN_PAGE_META_ROW}>
          ...
          {ADMIN_PAGE_SIZE_OPTIONS.map((size) => (
            <Link key={size} className={`${ADMIN_PAGE_ROWS_BUTTON} ${pageSize === size ? ADMIN_PAGE_ROWS_BUTTON_ACTIVE : ADMIN_PAGE_ROWS_BUTTON_IDLE}`} href={buildHref({ page_size: size, returns_page: 1 })}>
              {size}
            </Link>
          ))}
        </div>
