...
function getRentalHomeJsonLd(locale: string, tenant: TenantConfig) {
  ...
  const organization = {
    ...
    url: homeUrl,
    logo: tenant.logoUrl?.startsWith("http") ? tenant.logoUrl : `${baseUrl}${tenant.logoUrl || "/images/Logo.png"}`,
    email: tenant.email,
...
function getSaasHomeJsonLd(locale: string, tenant: TenantConfig) {
    ...
    offers: [
      {
        ...
        description: "One-time onboarding and setup cost to get started on the platform.",
      },
      {
        "@type": "Offer",
        priceCurrency: "USD",
        ...
        description: "Monthly subscription fee for full platform access.",
      },
    ],
    provider: {
      "@id": `${baseUrl}/#organization`,
  ...
  const reviews = [
    ...
    },
    {
      ...
      author: {
        "@type": "Organization",
        name: "Aloha Car Rental",
        url: "http://alohacarrentalbonaire.com",
      },
      reviewBody: "The best rental management solution for the Caribbean. Game changer!",
      publisher: {
        "@type": "Organization",
        name: tenant.tenantName,
      },
  ...
  return [organization, website, software, service, ...reviews];
}
